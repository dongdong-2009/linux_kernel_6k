...
*/
static void setup_pin_mux(struct pinmux_config *pin_mux)	//-‘À–––æ∆¨Ãÿ∂®µƒMUX≥ı ºªØ∫Ø ˝,◊Ó÷’–ﬁ∏ƒ¡Àºƒ¥Ê∆˜÷µ
{
	int i;
...
static void gpio_test_3352_T(int evm_id,int profile)
{
	setup_pin_mux(gpio_pin_mux_3352_T_);
...
static void gpio_init(int evm_id, int profile)
{
	...
	if(test_board)
		setup_pin_mux(gpio_pin_mux_3352_T);
	return;
}
...
//		.size			= 0x1b400000, /*512M - 4*128k*/
		...
		/*4ÂùóÁî®‰ΩúÂùèÂùóË°®ÔºåÂèØÁî®Â§ßÂ∞è‰∏∫Ôºösize = can-use - 4*128k*/
	},
};
...
//-‘⁄Õ¨“ª∏ˆ◊‹œﬂ…œ‘ˆº”…Ë±∏,’‚¿Ô”¶∏√æÕ «–≈œ¢ÃÌº”¥¶
static struct i2c_board_info am335x_i2c1_boardinfo[] = {//-≥ı ºªØ“ª∏ˆ¥”ª˙∆˜º˛–≈œ¢
	{
...
static void i2c1_init(int evm_id, int profile)
{
	...
	omap_register_i2c_bus(2, 400, am335x_i2c1_boardinfo,
			ARRAY_SIZE(am335x_i2c1_boardinfo));	//-◊¢≤·µΩ÷∏∂®µƒ◊‹œﬂ
	return;
}
...
static struct evm_dev_cfg zy_m3352_dev_cfg[] = {
	...
	{rgmii2_init,	DEV_ON_BASEBOARD, PROFILE_NONE }, //OKÔºåEPC-9600‰∏çÁî®ÂçÉÂÖÜÁΩë
...
static struct i2c_board_info __initdata am335x_i2c0_boardinfo[] = {
    {
        I2C_BOARD_INFO("ds2460", 0x80 >> 1),	//-ÃÌº”…Ë±∏–≈œ¢
...
//-≥ı ºªØi2c 0,”…”⁄i2c0“˝Ω≈µƒmode0æÕ «i2cπ¶ƒ‹£¨À˘“‘≤ª–Ë“™≈‰÷√“˝Ω≈∏¥”√
...
static void __init am335x_evm_init(void)	//-◊¢≤·¡Àø™∑¢∞ÂÀ˘”√µΩµƒÀ˘”–…Ë±∏µƒœ‡πÿ”≤º˛–≈œ¢
{
	int i;
...
//	platform_device_register(&serial_device_ex); //abing NOT-OK
	...
	/* ÂØºÂá∫gpio sysfs class */
	//GPIOs
/*
	if (gpio_request(75, "gpio75") < 0) {
		printk(KERN_ERR "failed to get GPIO_75\n");
	}
	gpio_export(75, 1);//1,ÂÖÅËÆ∏‰øÆÊîπÊñπÂêë
*/
	if (gpio_request(44, "gpio44") < 0) {
		printk(KERN_ERR "failed to get GPIO_44\n");
	}
	gpio_export(44, 1);//1,ÂÖÅËÆ∏‰øÆÊîπÊñπÂêë
	...
	if(test_board)
	{
		if (gpio_request(2, "gpio2") < 0) {
			printk(KERN_ERR "failed to get GPIO_2\n");
		}
		gpio_export(2, 1);

		if (gpio_request(3, "gpio3") < 0) {
			printk(KERN_ERR "failed to get GPIO_3\n");
		}
		gpio_export(3, 1);

		if (gpio_request(40, "gpio40") < 0) {
			printk(KERN_ERR "failed to get GPIO_40\n");
		}
		gpio_export(40, 1);

		if (gpio_request(41, "gpio41") < 0) {
			printk(KERN_ERR "failed to get GPIO_41\n");
		}
		gpio_export(41, 1);

		if (gpio_request(12, "gpio12") < 0) {
			printk(KERN_ERR "failed to get GPIO_12\n");
		}
		gpio_export(12, 1);
		...
		if (gpio_request(112, "gpio112") < 0) {
			printk(KERN_ERR "failed to get GPIO_112\n");
		}
		gpio_export(112, 1);

		if (gpio_request(116, "gpio116") < 0) {
			printk(KERN_ERR "failed to get GPIO_116\n");
		}
		gpio_export(116, 1);

		if (gpio_request(111, "gpio111") < 0) {
			printk(KERN_ERR "failed to get GPIO_111\n");
		}
		gpio_export(111, 1);

		if (gpio_request(115, "gpio115") < 0) {
			printk(KERN_ERR "failed to get GPIO_115\n");
		}
		gpio_export(115, 1);

		if (gpio_request(110, "gpio110") < 0) {
			printk(KERN_ERR "failed to get GPIO_110\n");
		}
		gpio_export(110, 1);

		if (gpio_request(114, "gpio114") < 0) {
			printk(KERN_ERR "failed to get GPIO_114\n");
		}
		gpio_export(114, 1);

		if (gpio_request(60, "gpio60") < 0) {
			printk(KERN_ERR "failed to get GPIO_60\n");
		}
		gpio_export(60, 1);

		if (gpio_request(117, "gpio117") < 0) {
			printk(KERN_ERR "failed to get GPIO_117\n");
		}
		gpio_export(117, 1);
	}
}
...
MACHINE_START(AM335XEVM, "3352-T")
	...
	.init_early		= am33xx_init_early,	//GPIOºƒ¥Ê∆˜µÿ÷∑÷∏∂®
	.init_irq		= ti81xx_init_irq,
	.handle_irq     = omap3_intc_handle_irq,
	.timer			= &omap3_am33xx_timer,
	.init_machine	= am335x_evm_init,	//GPIO“˝Ω≈∏¥”√≈‰÷√
MACHINE_END
