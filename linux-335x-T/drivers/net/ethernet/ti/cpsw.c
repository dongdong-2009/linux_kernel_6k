...
extern int davinci_mdio_write(struct mii_bus *bus, int phy_id, int phy_reg, u16 phy_data);
extern int davinci_mdio_read(struct mii_bus *bus, int phy_id, int phy_reg);
static bool link_flag = true;
	...
	if (phy->link) {
		...
		if (phy->speed == 100)
		{
			if (slave->phy->addr == 1) {
				if( link_flag ) {
					link_flag = false;
					/* turn on the led speed*/
					davinci_mdio_write(slave->phy->bus, slave->phy->addr, 0x18, 0x20);
				}
			}
			mac_control |= BIT(15);
		}
		...
		*link = true;
		...
		/* turn off the led speed*/
		davinci_mdio_write(slave->phy->bus, slave->phy->addr, 0x18, 0x24); //zp
		link_flag = true;
