...
EXPORT_SYMBOL(davinci_mdio_read);

//static int davinci_mdio_write(struct mii_bus *bus, int phy_id,
int davinci_mdio_write(struct mii_bus *bus, int phy_id,
...
EXPORT_SYMBOL(davinci_mdio_write);
...
static int __devinit davinci_mdio_probe(struct platform_device *pdev)
{
	...
	davinci_mdio_write(data->bus, 1, 0x18, 0x20); 

    //set MODE2 = LED_LINK on for good link,blink for activity
    ret = davinci_mdio_read(data->bus,1,0x19);              //zwj
    ret &= ~(1<<5);
    davinci_mdio_write(data->bus,1,0x19,ret);
